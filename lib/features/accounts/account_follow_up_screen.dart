...
class _AccountFollowUpScreenState extends State<AccountFollowUpScreen> with SingleTickerProviderStateMixin {
  ...
  // initialize with defaults; will be replaced by persisted values if present
...
Regards,''',
  ...
  void initState() {
    ...
    // load any persisted templates from settings; do not persist edits made here
    _loadTemplates();
  }
  ...
  // Load templates saved in Settings (note_initial, note_follow, note_final).
  // These values are used to populate the on-screen templates; edits on this screen are temporary
  // and won't be saved back to LocalStorage unless changed from Settings.
  Future<void> _loadTemplates() async {
    try {
      final vInitial = getString('note_initial');
      final vFollow = getString('note_follow');
      final vFinal = getString('note_final');

      if (!mounted) return;

      setState(() {
        _templates[0] = (vInitial?.isNotEmpty == true) ? vInitial! : _templates[0];
        _templates[1] = (vFollow?.isNotEmpty == true) ? vFollow! : _templates[1];
        _templates[2] = (vFinal?.isNotEmpty == true) ? vFinal! : _templates[2];
        if (!_isEditing) _draftCtrl.text = _buildTemplateText(_selectedIndex);
      });
    } catch (_) {
      // ignore load errors
    }
        ...
        actions: [
          PopupMenuButton<String>(
            ...
            onSelected: (v) async {
              ...
              if (v == 'edit') {
                if (a.isPaid) return;
                final res = await Navigator.of(context).push(MaterialPageRoute(builder: (_) => AddAccountScreen(account: a, asBottomSheet: true)));
